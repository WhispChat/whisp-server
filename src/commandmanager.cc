...
bool CommandManager::parse_command(Connection *conn, Command cmd) {
  ...
  if (type.compare("quit") == 0) {
    ...
    message_manager->send_and_create(server::Message::INFO,
                                     channel_list_message, conn);
  } else if (type.compare("create") == 0) {
    create_channel_command(conn, args);
  } else if (type.compare("join") == 0) {
    join_channel_command(conn, args);
  } else {
    std::string error_msg = "Unknown command";
    message_manager->send_and_create(server::Message::ERROR, error_msg, conn);
  }
...
bool CommandManager::login_command(Connection *conn,
  ...
  try {
    ...
    if (*conn->user == *found_user) {
      ...
      message_manager->send_and_create(server::Message::INFO, logged_in_msg,
                                       conn);
      return false;
    }

    if (!found_user || !found_user->compare_hash(password)) {
      message_manager->send_and_create(server::Message::ERROR,
    ...
    message_manager->send_and_create(server::Message::INFO, login_message,
                                     conn);

    LOG_INFO << "Connection " << *conn << " has changed auth" << '\n';
  } catch (const std::exception &ex) {
    message_manager->send_and_create(server::Message::ERROR, ex.what(), conn);
  }
...
void CommandManager::users_command(Connection *conn) {
  ...
  message_manager->send_and_create(server::Message::INFO, user_list_message,
                                   conn);
}

void CommandManager::create_channel_command(Connection *conn,
                                            std::vector<std::string> args) {
  ...
  if (duplicate_channel) {
    ...
    if (new_channel) {
      ...
      message_manager->send_and_create(server::Message::INFO, success_message,
                                       conn);
    } else {
      // SQLite error, inform user
...
void CommandManager::join_channel_command(Connection *conn,
                                          std::vector<std::string> args) {
  ...
  if (target_channel) {
    LOG_INFO << "target_channel: " << target_channel->name << '\n';
    ...
    if (channels.count(target_channel->name)) {
      ...
      if (target_channel->get_connection_amount() ==
        ...
        message_manager->send_and_create(server::Message::ERROR, error_msg,
                                         conn);
      }
    }
  } else {
